function ret = ex5_linear(frame_matrix, hop_size, window_type, noise_est, original_signal_length)
% Return enhanced signal after linear filtering
...
for winix = 1:size(frame_matrix, 2)
    ...
    pspec = (abs(Y_freq) - sqrt(noise_est(:, winix))) ./ abs(Y_freq);
    pspec(pspec < 0) = 0;
    pspec = Y_freq .* pspec;
    xwinest = ifft(pspec);
